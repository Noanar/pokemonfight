...
class Joueur:
    ...
    def choisir_pokemon(self, liste_pokemons):
        ...
        legendaire = False
        for _ in range(3):
            print(f"{self.nom} choisit un pokémon dans la liste ci-dessus : 0 à {length_liste_pokemons-1}")
    ...
    def choisir_attaque(self, pokemon):
        while True:
            try:
                ind_attaque = int(input(f"Que doit faire {pokemon.nom}? 0-{len(pokemon.attaques)-1} > "))
                if not (0 <= ind_attaque < len(pokemon.attaques)):
                    raise ValueError("Indice invalide.")
                attaque = pokemon.attaques[ind_attaque]
                if attaque.pp <= 0:
                    print(f"{attaque.nom} n'a plus de PP.")
                    continue
                attaque.pp -= 1  # décrémente les PP
                return attaque
            except ValueError as e:
                print(e)
...
class Pokemon:
    ...
    def afficher_attaques(self):
        for i, atk in enumerate(self.attaques):
            print(f"[{i}] ", end="")
            atk.afficher_attaque()
...
class Attaque:
    ...
    def calculer_degats(self, pokemon_attaquant, pokemon_attaque):
        stab = 1.5 if self.type == pokemon_attaquant.type else 1
        cm = stab * (self.precision / 100)
        pv_perdus = (((pokemon_attaquant.niveau * 0.4 + 2) *
                     pokemon_attaquant.attaque * self.puissance)
                     // (pokemon_attaque.defense * 50) + 2) * cm
        print(f"{pokemon_attaque.nom} perd {pv_perdus}")
        return pv_perdus
...
# TODO : classe Jeu
class Jeu:
    def __init__(self, liste_1, liste_2):
        self.l1 = liste_1
        self.l2 = liste_2
    ...
    def jouer(self):
        ...
        while len(joueur_1.pokemons) > 0 and len(joueur_2.pokemons) > 0:
            pokemon_j1 = joueur_1.pokemons[0]
            pokemon_j2 = joueur_2.pokemons[0]
            ...
            while not pokemon_j1.est_ko() and not pokemon_j2.est_ko():
                ...
                if pokemon_j1.vitesse >= pokemon_j2.vitesse:
                    pokemon_j2.pts_vie -= pokemon_j1.attaquer(pokemon_j2, attaque_pokemon_joueur1)
                    if not pokemon_j2.est_ko():
                        pokemon_j1.pts_vie -= pokemon_j2.attaquer(pokemon_j1, attaque_pokemon_joueur2)
                else:
                    pokemon_j1.pts_vie -= pokemon_j2.attaquer(pokemon_j1, attaque_pokemon_joueur2)
                    if not pokemon_j1.est_ko():
                        pokemon_j2.pts_vie -= pokemon_j1.attaquer(pokemon_j2, attaque_pokemon_joueur1)

            if pokemon_j1.est_ko():
                print(pokemon_j1.nom, "est KO.")
                joueur_2.manche_gagnee += 1
                joueur_1.pokemons.pop(0)
            if pokemon_j2.est_ko():
                print(pokemon_j2.nom, "est KO.")
                joueur_1.manche_gagnee += 1
                joueur_2.pokemons.pop(0)
